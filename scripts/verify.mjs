...
const consentGate = readFileSync('consent-gate.js', 'utf8');
...
assert(integrityJs.includes('SKY_SECURITY_READY'), 'Security integrity marker missing');
assert(integrityJs.includes('SKY_SECURITY_BLOCKED'), 'Fail-closed security block missing');
assert(gameJs.includes('SKY_SECURITY_HEARTBEAT'), 'Anti-cheat heartbeat missing');
assert(gameJs.includes('SKY_SECURITY_BLOCKED'), 'Game must block when security runtime fails');
assert(consentGate.includes('pauseAdRequests = 1'), 'Ad requests must be paused before consent');
assert(consentGate.includes('__tcfapi'), 'TCF consent listener missing');
assert(gameJs.includes('consent_required'), 'AdSense must require consent');
assert(gameJs.includes('ADSENSE_PUBLISHER_ID'), 'AdSense publisher ID must come from public build config');
...
assert(readFileSync('scripts/build-web.mjs', 'utf8').includes("'security-integrity.js'"), 'Integrity guard must be copied to dist');
...
assert(readFileSync('supabase/schema.sql', 'utf8').includes('create table if not exists public.score_runs'));
assert(readFileSync('.env.example', 'utf8').includes('VITE_PUBLIC_SITE_URL='));
assert(readFileSync('.env.example', 'utf8').includes('VITE_SUPABASE_URL='));
assert(readFileSync('.env.example', 'utf8').includes('VITE_SUPABASE_ANON_KEY='));
...
assert(vercelApi.includes("import { app } from '../../server/app.mjs'"), 'Vercel v2 API adapter missing');
assert(vercel.includes('api/v1/[...path].mjs'), 'Vercel v2 function configuration missing');
...
assert(security.includes('SUPABASE_URL'));
...
console.log(`verified HTML + ui.js + consent-gate.js + game.js; ${scripts.length} inline metadata script block(s)`);
