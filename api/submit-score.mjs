...
import { consumeScoreRun, findScoreRun, getLeaderboard, insertScore } from './_db.mjs';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const run = verifyRunTicket(payload.ticket, session.user.id);
    ...
    if (payload.score > maxPlausibleScore) return json(res, 422, { error: 'score_rate_exceeded' });

    const existing = await findScoreRun({ runId: run.runId, userId: session.user.id });
    if (!existing || existing.submitted_at) return json(res, 409, { error: 'run_already_used' });

    const lockedRows = await consumeScoreRun({ runId: run.runId, userId: session.user.id, submittedAt: new Date(now).toISOString() });
    if (lockedRows.length !== 1) return json(res, 409, { error: 'run_already_used' });

    await insertScore({ name: payload.name, score: payload.score, userId: session.user.id });
    return json(res, 200, { rows: await getLeaderboard() });
  } catch (error) {
    return json(res, error instanceof SyntaxError ? 400 : error.status || 500, { error: error instanceof SyntaxError ? 'invalid_json' : error.status === 500 ? 'server_not_configured' : 'request_failed' });
  }
}
