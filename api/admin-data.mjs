    ...
    try {
      ...
      if (session.db && !session.supabase) {
        const [scores, scoreCount, runCount, submittedCount, topScore] = await Promise.all([
          session.db`SELECT id, player_name, score, user_id, created_at FROM scores ORDER BY created_at DESC LIMIT 100`,
        ...
        return json(res, 200, { generatedAt: new Date().toISOString(), stats: { scoreCount: scoreCount[0]?.count || 0, runCount: runCount[0]?.count || 0, submittedRunCount: submittedCount[0]?.count || 0, topScore: topScore[0]?.score || 0 }, scores });
      }

      const snapshot = await getAdminSnapshot();
      return json(res, 200, { generatedAt: new Date().toISOString(), ...snapshot });
    } catch (error) {
      return json(res, error.status || 500, { error: error.status === 403 ? 'forbidden' : error.status === 500 ? 'server_not_configured' : 'admin_request_failed' });
    }
  };
}
