...
Object.keys(LANG).forEach(function(k){LANG[k].rankLabel=LANG[k].rankLabel||'hạng';LANG[k].playerLabel=LANG[k].playerLabel||'người chơi';LANG[k].scoreLabel=LANG[k].scoreLabel||'điểm';LANG[k].refresh=LANG[k].refresh||'làm mới';});LANG.en.rankLabel='rank';LANG.en.playerLabel='player';LANG.en.scoreLabel='score';LANG.en.refresh='refresh';LANG.ja.rankLabel='順位';LANG.ja.playerLabel='プレイヤー';LANG.ja.scoreLabel='得点';LANG.ja.refresh='更新';LANG.vi.topFlights='10 người chơi có điểm cao nhất';
...
function renderLb(el,rows,maxN,hl){
  ...
  for(var i=0;i<n;i++){var r=rows[i];var me=hl&&rank<0&&r.name===hl;if(me)rank=i;
    ...
    d.setAttribute('role','row');a.setAttribute('role','cell');b.setAttribute('role','cell');e.setAttribute('role','cell');d.appendChild(a);d.appendChild(b);d.appendChild(c);d.appendChild(e);el.appendChild(d);}
  return rank;
}
