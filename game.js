...
var DB={
  ...
  init:function(){
    ...
    .then(function(mod){if(!mod||typeof mod.createClient!=='function')throw new Error('supabase driver');var isNative=!!(window.SKY_NATIVE&&window.SKY_NATIVE.isNative);DB.client=authClient=mod.createClient(SUPABASE_URL,SUPABASE_ANON_KEY,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:!isNative}});DB.client.auth.onAuthStateChange(function(event,session){authUser=session&&session.user||null;updateAuthUI();});if(window.SKY_NATIVE&&window.SKY_NATIVE.onAuthCallback)window.SKY_NATIVE.onAuthCallback(function(url){handleNativeAuthCallback(url).catch(function(){});});return handleNativeAuthCallback(window.SKY_NATIVE&&window.SKY_NATIVE.getAuthCallback&&window.SKY_NATIVE.getAuthCallback()).catch(function(){return null;}).then(function(){return DB.client.auth.getSession();});})
