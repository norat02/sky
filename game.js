...
function renderLb(el,rows,maxN,hl){
  ...
  for(var i=0;i<n;i++){var r=rows[i];var me=hl&&rank<0&&r.name===hl;if(me)rank=i;
    ...
    var share=document.createElement('button');share.type='button';share.className='lbShare';share.textContent=tx('share')||'chia sẻ';share.setAttribute('aria-label',(tx('shareScore')||'chia sẻ điểm')+' '+String(r.name||'')+' '+r.score);share.addEventListener('click',function(){shareScore(String(r.name||'').slice(0,12),r.score,i+1);});
